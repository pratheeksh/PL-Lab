...
abstract class Tree[+T] {
 ...
 def map[T2](function: (T) => T2): Tree[T2]
}
...
case class Leaf[T](x:T) extends Tree[T] {
  ...
  override def map[T2](function: (T) => T2): Tree[T2] = {
    return new Leaf(function(x))
...
case class Node[T](x:T,children:List[Tree[T]]) extends Tree[T] {
  ...
  override def map[T2](function: T => T2): Tree[T2] = {
    return new Node(function(x),children.map(t => t.map(function)))

  }
  override def toString(): String = "Node(" + x + ")," + "List(" + children.map(t => t.toString()) + ")"
...
class A(x:Int) {
  def value: Int =  x
  def plus(other:A): A =  new A(this.value + other.value)
...
//      breadth [tr] []
//   end
  def breadthFirst(tree: Tree[A ]): List[A] = {

    var res:List[A] = Nil
    var queue:List[Tree[A]] = Nil
      tree match {
        case Leaf(v:A) => return List(v)
        case Node(v:A, children:List[Tree[A]]) => {
          res = v::res
          queue = queue:::children
        }
      }
      def bfsLoop (start: List[A], nextLayer: List[Tree[A]]): List[A] = nextLayer match {
        ...
        case _ => {
          ...
          for (t:Tree[A] <- nextLayer){
            t match {
              case Leaf(v:A) =>             res = v::res
       ...
       return bfsLoop(res,queue).reverse
...
// Be sure to make reduce as polymorphic as possible.
  ...
  def reduce(function:(A,A)=>A,l:List[A],b: A ) : A = {
    if (l.size == 0){
      return b
    }
    else{
      return function(b,reduce(function,l.tail,l.head))
    }
...
// function.
  ...
  def sumTreeA(t: Tree[A]): A ={
    ...
    return reduce(sum,breadthFirst(t),new A(0))
